...
const TournamentBracket = () => {
  ...
  const fetchTournament = async () => {
    try {
      ...
      if (data.matches.length > 0) {
        const rounds = Math.max(...data.matches.map(m => m.round_number || 1));
        setMaxRound(rounds);
      }
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateScore = async (matchId) => {
    try {
      await api.post(`/matches/update-details?match_id=${matchId}`, {
        score_home: parseInt(scoreForm.home),
        score_away: parseInt(scoreForm.away),
        status: 'finished'
      });
  ...
  const roundMatches = tournament.matches.filter(m => m.round_number === currentRound);
  const isFinal = currentRound === maxRound;
  
  // تحديد البطل إذا انتهت النهائي
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white font-sans overflow-x-auto pb-10">
      ...
      <div className="relative bg-gradient-to-b from-blue-900 to-slate-900 p-8 text-center border-b border-blue-700 shadow-2xl min-w-[800px]">
        <button onClick={() => navigate(`/tournament/${id}`)} className="absolute top-4 left-4 text-gray-400 hover:text-white transition">
          🔙 عودة للإدارة
        </button>
        
        <h1 className="text-4xl font-bold mb-2 tracking-wider uppercase">{tournament.name}</h1>
        <p className="text-blue-300 mb-6">نظام خروج مغلوب احترافي</p>
        ...
        {champion && (
          <div className="mt-4 bg-yellow-500 text-black px-8 py-3 rounded-full inline-block font-bold text-2xl shadow-lg animate-pulse">
            🏆 البطل: {champion} 🏆
          </div>
        )}
      </div>

      {/* شريط التنقل بين الأدوار */}
      <div className="sticky top-0 z-20 bg-slate-800/95 backdrop-blur p-4 flex justify-center items-center gap-6 border-b border-slate-700 shadow-lg min-w-[800px]">
        ...
        <span className="text-xl font-bold text-blue-400 bg-slate-900 px-8 py-2 rounded-xl border border-blue-900 shadow-inner">
          {currentRound === 1 ? 'دور الـ 16' : 
           currentRound === 2 ? 'ربع النهائي' : 
           currentRound === 3 ? 'نصف النهائي' : 'المباراة النهائية'}
        </span>
        ...
        {roundMatches.length === 0 ? (
          ...
          roundMatches.map((match) => (
            <div key={match.id} className="relative w-full max-w-2xl bg-slate-800 rounded-xl overflow-hidden shadow-xl border border-slate-700 hover:border-blue-500 transition-all duration-300 group">
                ...
                <div className="flex justify-between items-center p-5 bg-slate-800/50">
                  ...
                  {editingMatch === match.id ? (
                    <input 
                      ...
                      onChange={(e) => setScoreForm({...scoreForm, home: e.target.value})}
                      className="w-14 text-center bg-slate-900 border-2 border-blue-500 rounded-lg text-white font-bold text-xl p-1"
                    />
                  ) : (
                ...
                <div className="flex justify-between items-center p-5 bg-slate-800/50">
                  ...
                  {editingMatch === match.id ? (
                    <input 
                      ...
                      onChange={(e) => setScoreForm({...scoreForm, away: e.target.value})}
                      className="w-14 text-center bg-slate-900 border-2 border-blue-500 rounded-lg text-white font-bold text-xl p-1"
                    />
                  ) : (
                    <span className={`text-2xl font-bold ${match.status === 'finished' ? 'text-white' : 'text-gray-600'}`}>
              ...
              {/* طبقة التفاعل (تظهر عند Hover) */}
              <div className="absolute inset-0 bg-black/70 flex items-center justify-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 backdrop-blur-sm">
                {editingMatch === match.id ? (
                  <>
                    <button onClick={() => handleUpdateScore(match.id)} className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg transform hover:scale-105 transition">
                      حفظ
                    </button>
                    <button onClick={() => setEditingMatch(null)} className="bg-red-600 hover:bg-red-500 text-white px-6 py-2 rounded-lg font-bold shadow-lg">
                      إلغاء
                    </button>
                  </>
                ) : (
                  <button 
                    onClick={() => { setEditingMatch(match.id); setScoreForm({home: match.score_home, away: match.score_away}); }}
                  ...
                  >
                    {match.status === 'finished' ? '✏️ تعديل' : '⚽ إدخال النتيجة'}
                  </button>
                )}
              </div>
            </div>
